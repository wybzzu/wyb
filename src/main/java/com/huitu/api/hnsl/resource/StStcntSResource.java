...
@Path("stStcntS") 
public class StStcntSResource extends AbstractApiResource{  

	//@Autowired
	private StStcntSService service;
	
	@Autowired
	private RedisTemplate redisTemplate;
	
	public void setService(StStcntSService service){
		this.service=service;
	}
	
	public StStcntSService getService(){
		return this.service;
	}
     ...
     * Description: TODO(条件删除StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiDescription 
     * @apiVersion 0.1
     * @apiName get StStcntS 
     * @apiGroup @@@
     *
     * @apiParam {Object} StStcntSExample
     * @apiParam {string} StStcntSExample.@@@ @@@
     *
    ...
    public Response deleteByExample(StStcntS record){
    	try{
     ...
     * Description: TODO(单主键删除StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    /**
     * Title: insert </BR>
     * Description: TODO(插入StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiGroup @@@
     *
     * @apiParam {Object} StStcntS
     * @apiParam {string} StStcntS.@@@ @@@
     *
    ...
    @Path("/insert")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insert(StStcntS record){
    	try{
     ...
     * Description: TODO(选择非空插入StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insertSelective(StStcntS record){
    	try{
	    	int i=service.insert(record);
	    	map.put("effective",i);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
     ...
     * Description: TODO(条件查询StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectByExample(StStcntS record){
    	try{
	    	List<StStcntS> list=service.selectByExample(record);
	    	map.put("result",list);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
	        response =Response.ok(sr).status(Response.Status.OK).build();
        }
     ...
     /**
     * Title: selectAll </BR>
     * Description: TODO(条件分页查询StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Path("/selectAll")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectAll(StStcntS record){
        ...
        try {
            ...
            if (StringUtils.isNoneBlank(rang)) {
                ...
                String hash = UUIDFactory.hashUUID("stStcntS");
                if(cache!=null&&Boolean.getBoolean(cache)){
            		 list=service.selectByExample(record);
                     RedisClient.setHashValue(redisTemplate, "stStcntS", hash, list, NeverExpirable.Never);
                     ...
                     sr = new PagingResult<List<StStcntS>>(this, list.subList(min, max), paging);
                     response = Response.ok(sr).status(Response.Status.OK).build();
            	}else{
                    PageHelper.startPage(page.getPageNum(), page.getSize());
                    list=service.selectByExample(record);
                    Paging paging=PageUtil.getPaging(list.size(),hash,"be_adcd_b",page.getSize(),page.getMax());
                    sr = new PagingResult<List<StStcntS>>(this, list, paging);
                    response = Response.ok(sr).status(Response.Status.OK).build();
            	}
            } else {
            	list=service.selectByExample(record);
                sr = new SuccessResult<List<StStcntS>>(this, list);
                response = Response.ok(sr).status(Response.Status.OK).build();
            }
     ...
     * Description: TODO(主键查询StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    public Response selectByPrimaryKey(@PathParam("id") String id){
        try{
	    	StStcntS obj=service.selectByPrimaryKey(id);
	    	map.put("obj",obj);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
	        response =Response.ok(sr).status(Response.Status.OK).build();
        }
   ...
   /**
     ...
     * Description: TODO(主键更新且地段非空StStcntS ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response updateByPrimaryKeySelective(StStcntS record){
    	try{
