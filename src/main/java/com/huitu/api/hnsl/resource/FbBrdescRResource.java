...
@Path("fbBrdescR") 
public class FbBrdescRResource extends AbstractApiResource{  

	//@Autowired
	private FbBrdescRService service;
	
	@Autowired
	private RedisTemplate redisTemplate;
	
	public void setService(FbBrdescRService service){
		this.service=service;
	}
	
	public FbBrdescRService getService(){
		return this.service;
	}
     ...
     * Description: TODO(条件删除FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiDescription 
     * @apiVersion 0.1
     * @apiName get FbBrdescR 
     * @apiGroup @@@
     *
     * @apiParam {Object} FbBrdescRExample
     * @apiParam {string} FbBrdescRExample.@@@ @@@
     *
    ...
    public Response deleteByExample(FbBrdescR record){
    	try{
     ...
     * Description: TODO(单主键删除FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    /**
     * Title: insert </BR>
     * Description: TODO(插入FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiGroup @@@
     *
     * @apiParam {Object} FbBrdescR
     * @apiParam {string} FbBrdescR.@@@ @@@
     *
    ...
    @Path("/insert")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insert(FbBrdescR record){
    	try{
     ...
     * Description: TODO(选择非空插入FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insertSelective(FbBrdescR record){
    	try{
	    	int i=service.insert(record);
	    	map.put("effective",i);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
     ...
     * Description: TODO(条件查询FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectByExample(FbBrdescR record){
    	try{
	    	List<FbBrdescR> list=service.selectByExample(record);
	    	map.put("result",list);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
	        response =Response.ok(sr).status(Response.Status.OK).build();
        }
     ...
     /**
     * Title: selectAll </BR>
     * Description: TODO(条件分页查询FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Path("/selectAll")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectAll(FbBrdescR record){
        ...
        try {
            ...
            if (StringUtils.isNoneBlank(rang)) {
                ...
                String hash = UUIDFactory.hashUUID("fbBrdescR");
                if(cache!=null&&Boolean.getBoolean(cache)){
            		 list=service.selectByExample(record);
                     RedisClient.setHashValue(redisTemplate, "fbBrdescR", hash, list, NeverExpirable.Never);
                     ...
                     sr = new PagingResult<List<FbBrdescR>>(this, list.subList(min, max), paging);
                     response = Response.ok(sr).status(Response.Status.OK).build();
            	}else{
                    PageHelper.startPage(page.getPageNum(), page.getSize());
                    list=service.selectByExample(record);
                    Paging paging=PageUtil.getPaging(list.size(),hash,"be_adcd_b",page.getSize(),page.getMax());
                    sr = new PagingResult<List<FbBrdescR>>(this, list, paging);
                    response = Response.ok(sr).status(Response.Status.OK).build();
            	}
            } else {
            	list=service.selectByExample(record);
                sr = new SuccessResult<List<FbBrdescR>>(this, list);
                response = Response.ok(sr).status(Response.Status.OK).build();
            }
     ...
     * Description: TODO(主键查询FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    public Response selectByPrimaryKey(@PathParam("id") String id){
        try{
	    	FbBrdescR obj=service.selectByPrimaryKey(id);
	    	map.put("obj",obj);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
	        response =Response.ok(sr).status(Response.Status.OK).build();
        }
   ...
   /**
     ...
     * Description: TODO(主键更新且地段非空FbBrdescR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response updateByPrimaryKeySelective(FbBrdescR record){
    	try{
