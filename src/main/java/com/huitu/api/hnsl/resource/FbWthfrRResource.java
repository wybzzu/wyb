...
@Path("fbWthfrR") 
public class FbWthfrRResource extends AbstractApiResource{  

	//@Autowired
	private FbWthfrRService service;
	
	@Autowired
	private RedisTemplate redisTemplate;
	
	public void setService(FbWthfrRService service){
		this.service=service;
	}
	
	public FbWthfrRService getService(){
		return this.service;
	}
     ...
     * Description: TODO(条件删除FbWthfrR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiDescription 
     * @apiVersion 0.1
     * @apiName get FbWthfrR 
     * @apiGroup @@@
     *
     * @apiParam {Object} FbWthfrRExample
     * @apiParam {string} FbWthfrRExample.@@@ @@@
     *
    ...
    public Response deleteByExample(FbWthfrR record){
    	try{
    ...
    /**
     * Title: insert </BR>
     * Description: TODO(插入FbWthfrR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiGroup @@@
     *
     * @apiParam {Object} FbWthfrR
     * @apiParam {string} FbWthfrR.@@@ @@@
     *
    ...
    @Path("/insert")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insert(FbWthfrR record){
    	try{
     ...
     * Description: TODO(选择非空插入FbWthfrR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insertSelective(FbWthfrR record){
    	try{
	    	int i=service.insert(record);
	    	map.put("effective",i);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
     ...
     * Description: TODO(条件查询FbWthfrR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectByExample(FbWthfrR record){
    	try{
	    	List<FbWthfrR> list=service.selectByExample(record);
	    	map.put("result",list);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
	        response =Response.ok(sr).status(Response.Status.OK).build();
        }
     ...
     /**
     * Title: selectAll </BR>
     * Description: TODO(条件分页查询FbWthfrR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Path("/selectAll")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectAll(FbWthfrR record){
        ...
        try {
            ...
            if (StringUtils.isNoneBlank(rang)) {
                ...
                String hash = UUIDFactory.hashUUID("fbWthfrR");
                if(cache!=null&&Boolean.getBoolean(cache)){
            		 list=service.selectByExample(record);
                     RedisClient.setHashValue(redisTemplate, "fbWthfrR", hash, list, NeverExpirable.Never);
                     ...
                     sr = new PagingResult<List<FbWthfrR>>(this, list.subList(min, max), paging);
                     response = Response.ok(sr).status(Response.Status.OK).build();
            	}else{
                    ...
                    sr = new PagingResult<List<FbWthfrR>>(this, list, paging);
                    response = Response.ok(sr).status(Response.Status.OK).build();
            	}
            } else {
            	list=service.selectByExample(record);
                sr = new SuccessResult<List<FbWthfrR>>(this, list);
                response = Response.ok(sr).status(Response.Status.OK).build();
            }
   ...
   /**
     ...
     * Description: TODO(主键更新且地段非空FbWthfrR ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response updateByPrimaryKeySelective(FbWthfrR record){
    	try{
