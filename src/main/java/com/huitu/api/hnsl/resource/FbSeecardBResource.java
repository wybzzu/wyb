...
@Path("fbSeecardB") 
public class FbSeecardBResource extends AbstractApiResource{  

	//@Autowired
	private FbSeecardBService service;
	
	@Autowired
	private RedisTemplate redisTemplate;
	
	public void setService(FbSeecardBService service){
		this.service=service;
	}
	
	public FbSeecardBService getService(){
		return this.service;
	}
     ...
     * Description: TODO(条件删除FbSeecardB ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiDescription 
     * @apiVersion 0.1
     * @apiName get FbSeecardB 
     * @apiGroup @@@
     *
     * @apiParam {Object} FbSeecardBExample
     * @apiParam {string} FbSeecardBExample.@@@ @@@
     *
    ...
    public Response deleteByExample(FbSeecardB record){
    	try{
    ...
    /**
     * Title: insert </BR>
     * Description: TODO(插入FbSeecardB ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
     ...
     * @apiGroup @@@
     *
     * @apiParam {Object} FbSeecardB
     * @apiParam {string} FbSeecardB.@@@ @@@
     *
    ...
    @Path("/insert")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insert(FbSeecardB record){
    	try{
     ...
     * Description: TODO(选择非空插入FbSeecardB ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response insertSelective(FbSeecardB record){
    	try{
	    	int i=service.insert(record);
	    	map.put("effective",i);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
     ...
     * Description: TODO(条件查询FbSeecardB ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectByExample(FbSeecardB record){
    	try{
	    	List<FbSeecardB> list=service.selectByExample(record);
	    	map.put("result",list);
	        sr = new SuccessResult<Map< String, Object >>(this,map);
	        response =Response.ok(sr).status(Response.Status.OK).build();
        }
     ...
     /**
     * Title: selectAll </BR>
     * Description: TODO(条件分页查询FbSeecardB ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Path("/selectAll")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response selectAll(FbSeecardB record){
        ...
        try {
            ...
            List<FbSeecardB> list;
            if (StringUtils.isNoneBlank(rang)) {
                Page page=PageUtil.getPage(rang);
                String hash = UUIDFactory.hashUUID("fbSeecardB");
                if(cache!=null&&Boolean.getBoolean(cache)){
            		 list=service.selectByExample(record);
                     RedisClient.setHashValue(redisTemplate, "fbSeecardB", hash, list, NeverExpirable.Never);
                     ...
                     sr = new PagingResult<List<FbSeecardB>>(this, list.subList(min, max), paging);
                     response = Response.ok(sr).status(Response.Status.OK).build();
            	}else{
                    ...
                    sr = new PagingResult<List<FbSeecardB>>(this, list, paging);
                    response = Response.ok(sr).status(Response.Status.OK).build();
            	}
            } else {
            	list=service.selectByExample(record);
                sr = new SuccessResult<List<FbSeecardB>>(this, list);
                response = Response.ok(sr).status(Response.Status.OK).build();
            }
   ...
   /**
     ...
     * Description: TODO(主键更新且地段非空FbSeecardB ) </BR>
     * @return </BR>   
     * return Response </BR>   
     * throws </BR>
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response updateByPrimaryKeySelective(FbSeecardB record){
    	try{
